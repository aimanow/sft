<template>
  <div class="aspect_item " :class="{'active': active}" @click.prevent="toggle">
    <a class="aspect_item_img">
      ...
      <div class="aspect_item_text" :class="{'active': small}"><span class="icon-check"></span>
        <p>{{$lang.aspects[item.title] || item.title}}</p></div>
    </a>
  </div>
</template>
  ...
  methods: {
    toggle() {
      ...
      if (this.active) {
        ...
        this.$emit('checkOffAspect', this.item.id)
      }
    }
  }
}
